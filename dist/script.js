...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      postDate('assets/server.php', FormData).then(res => {
        console.log(res);
        statusMessage.textContent = message.success;
      }).catch(() => statusMessage.textContent = message.error).finally(() => {
        clearInput();
        setTimeout(() => {
...
const timer = (id, deadline) => {
  ...
  const setClock = (selector, endtime) => {
    ...
    function updateClock() {
      ...
      days.textContent = addZero(t.days), hours.textContent = addZero(t.hours);
      minutes.textContent = addZero(t.minutes);
      seconds.textContent = addZero(t.seconds);

      if (t.total <= 0) {
        days.textContent = '00', hours.textContent = '00', minutes.textContent = '00', seconds.textContent = '00', clearInterval(timeInterval);
      }
    }
  };
